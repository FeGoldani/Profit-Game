...
window.addEventListener("DOMContentLoaded", function() {
    ...
    var LOOP_VOLUME = 0.2;
    var BACKGROUND_SPEED = 2;//altera a velocidade da tela de fundo
    var BACKGROUND_HEIGHT = 1200;
    var MOVEMENT_RATE = 15;
    var KEY_ESC = 27;
    var MAX_LIVES = 3;
    var LIFE_DELAY = 60;
    var INVULNERABLE_PERIOD = 30;
    ...
    Explosion.WIDTH = 64;
    Explosion.HEIGHT = 64;
    Explosion.FRAMES = 17;//16; mostra o número de frames da explosão
    ...
    function Explosion(deadSprite) {
        ...
        this.w = Math.floor(deadSprite.w * (4/4));//(4/3));altera o tamanho dos frames da explosão
        this.h = Math.floor(deadSprite.h * (4/4));//(4/3));
    ...
    Starship.HP = 100;//controla a energia da nave
    Starship.DAMAGE = 100;
    Starship.FIRE_DELAY = 3;//controla o tempo entre um tiro e outro da nave espacial
    ...
    EnemyUFO.HEIGHT = 48;
    EnemyUFO.SPEED = 1;//2;
    EnemyUFO.GENERATION_RATE = [1/6,1/6,1/6];//[1/6, 1/5, 1/4];
    EnemyUFO.ROTATION_RATE = 6;
    EnemyUFO.HP = 100;
    EnemyUFO.DAMAGE = 1;//30;
    EnemyUFO.POINTS = 1;//20;
	...
	// função para retornar da tela de instruções FG
	...
	// função nova para mostrar Instruções	FG
	function showInst() { 
        document.getElementById("start").style.visibility = "hidden";
        document.getElementById("inst").style.visibility = "visible";
        window.removeEventListener("keydown", startHandler, false);
        window.addEventListener("keydown", instHandler, false);
    }
    //fim da função FG
    
    function returnToStart() {
        document.getElementById("inst").style.visibility = "hidden"; // retorna da tela de instruções FG
        document.getElementById("credits").style.visibility = "hidden";
        document.getElementById("start").style.visibility = "visible";
		window.removeEventListener("keydown", instHandler, false); // retorna da tela de instruções FG
	...
	// chamada do Inst FG
	document.getElementById("inst_link").addEventListener("click", showInst, false); 
    ...
    // evento para clicar no Link inst FG
	inst.addEventListener("click", function(event) { 
        if (event.target.tagName != "A")
            returnToStart();
    }, false);
    // fim do evento FG	
